...
### IV. Next Step
  ...
  Our preliminary analysis revealed clear differences in heart attack prevalence across `BMI` levels and demographic variables. Specifically, higher `BMI` values were associated with a greater proportion of heart attack cases, and similar trends were observed for older `AGE`, lower `INCOME`, being male (`SEX`), and certain `RACE` groups. These patterns suggest that `BMI` may be an important predictor of heart attack risk, and that demographic factors may confound or modify this relationship.  
  ...
  In the main logistic regression model, `BMI` will be the sole predictor, with its statistical significance assessed using individual *Z*-tests. The extended model will add `SEX`, `INCOME`, `RACE`, and `AGE` as covariates. Because these covariates are categorical, we will evaluate how the odds of `HeartAttack` change between categories. Their overall contribution will be assessed with partial $\chi^2$ tests, and differences between levels will be further examined through individual *Z*-tests. Variables that do not show significant differences will be excluded from the final model.  

  The final extended model will be validated for robustness: influential data points will be identified using Cook’s distance, regression outliers will be assessed with outlier tests, and multicollinearity will be checked using **Variance Inflation Factor (VIF)**. Model performance will be evaluated using goodness-of-fit measures, including the error rate test, to assess predictive accuracy.
<br><br>