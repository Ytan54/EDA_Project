...
At the end of our initial environment preparing section, we load our dataset into R Studio, while defining our relevant predictors (or independent variables), along with our dependent variable, for the following set of data analysis.
...
This marks the end of the first chunk of code - initializing and environment-defining. This chunk allows us to make sure that all of our data, functions, and relevant variables are ready to be used in other chunks that might call upon them for data analysis. By ensuring that each of these steps gets executed, we know that no matter where this code is run from, it will always have the same reliable and working environment to call upon.
<br><br>
...
**Conclusion**: We found that this Bernoulli distribution slightly leans more towards the female sex for this research database and subject study. This is something notable to consider when looking at other results from distributions and linear regression, as the subject's sex isn't evenly distributed, rather leaning more towards a higher count of female participants than of the male sex.
<br><br>
...
**Conclusion**: The Income distribution seems to be an inverse exponential distribution, with most subjects (or the mode) belonging to the income group of over 75 thousand dollars in income per year. When looking at the mean that we calculated earlier, we can see that its results in 35-50k which makes sense when looking at the distribution itself. This means that we have a lot more subjects that had a higher income value than other income groups, which should be noted when looking at other peices of data in this project, as a higher income will allow for greater access to quality healthcare and insuarance.
<br><br>
...
**Conclusion**: When looking at the predictor of Race, the distribution also appears to be a unimodal exponential distribution, with the mode of the data belonging to the race group of "White". This distribution also tells us to be weary for the other parts of our study, as there isn't a proportionate distribution of all groups, which limits the results of our findings to other groups of the predictor.
<br><br>
...
**Conclusion**: This Age distribution resembles that of a Poission distribution, which while more skwered to older ages with a median of 55-59, has a better distribution of all ages from 18 to 81+. This makes our results much more comprehensive for ages across our range, but with a noticable skew towards older ages, which should be noted when looking at other results.
<br><br>
...
**Conclusion**: We find that this distrubtion matches that of a binomial distribution, with a large tail that stretches towards the higher BMI values. This means that we have a very comprehensive distribution for the lower BMI values, but due to the large tail and few values at the higher end of range, our statistics such as mean and median are being skewed to the right. 
<br><br>
...
**Conclusion**: This distribution matches that of a Bernuolli distribution, but with more options that are being displayed in the graph rather than the typical two. There is an overwhelming more proportion of subjects that picked the category "Yes", but the mode still is "Borderline", which means that while we do not have many subjects that are in the "No" category, there are a lot in-between that might tip the distribution to be more equal than we initially thought.
<br><br>
...
**Conclusion**: This is a simple Bernoulli distribution, with a skewed distribution towards "Yes". This is another thing to take note of when we do further data analysis later on, in which the "No" cateogry is not fairly represented in this database, which can very well skew results.
<br><br>
...
**Conclusion**: And lastly,the Excercise perdictor is also a simple Bernoulli distribution, with a skewed distribution towards "Yes". This is another thing to take note of when we do further data analysis later on, in which the "No" cateogry is not fairly represented in this database, which can very well skew results. This means that most of our subjects in the database do excericse, which can drastically impact the results we find.
<br><br>
...
**Conclusion**: When looking at the bar plot we made, we do not see a noticeable change between the different catgeories of the Race predictor and the proportion of those who said yes to having a heart attack. This however could be skewed, as when looking at the Race predictor's distribution, we see a very skewed and uneven distribution of participants, with lots of missing datapoints for different catgeories.
<br><br>
...
**Conclusion**: Here we see a very exponential correlation between Age and the proportion of heart attacks. As the ages get higher and higher, so does the proportion of participants who noted that they did have a heart attack, rising exponentially within the older ages at the end of our sample range.
<br><br>
...
**Conclusion**: Very interestingly, we do not see much of a difference in poportions of heart attacks across the different categories of BMI. It should be noted that the BMI predictor did not have many samples in the categories of "Obese" and "Overweight", which could lead to a biased statistic that needs more samples in order to truly tell us if there is no correlation.
<br><br>
...
**Conclusion**: We conclude that there is a correlation between the different categories of Blood Pressure and the proportion of heart attacks. The "Yes" category has a much higher proportion of heart attacks than the others, with a value of 0.107 compared to 0.019, 0.025, and 0.037, which is a major different in proportions.
<br><br>
...
**Conclusion**: There doesn't appear to be a correlation between Healthcare Coverage and the proportion of heart attacks. Both categories have very similar poportions, with only a slight increase for the "Yes" category, but not a statistcally significant one. Therfore, we deduce there is not a corelation with this predictor
<br><br>
...
**Conclusion**: We do see a correlation bweteen Excerise activity and the proportion of heart attacks. The "No" category has double the proportion of heart attacks as the "Yes" category, which leads us to conclude that there is a strong correlation between the activity and excerise rate and heart attacks.
<br><br>
...
### IV. Future Directions
  ...
  In the bar graphs, for each predictor, we visualize proportion of individuals with heart attack across different categories. Based on this preliminary analysis, we have observed correlations between all selected predictors with response variable **HeartAttack**. Proportions of individuals with heart attack vary across different categorical levels, and it is observed in all predictors. Thus, in the logistic regression, we will use all predictors in the logistic regression model. 

  Specifically, we set **BMI_Num** as the predictor in the main model. Because **BMI_Num** is a numeric variable, we will analyze how the odds of having heart attack change with a one-unit increase in BMI. The significance of each predictor’s relationship with heart attack will be assessed using individual Z-tests. 

  Other categorical factors will add as additive predictors in the extended model, which include **SEX**, **Income**, **RACE**, **AGE**, **BloodPressure**, **HealthCare**, **Exercise**. Because they are categorical variables, we will analyze how the odds of having heart attack change when the category changes within each variable. Their overall contribution will first be evaluated using partial $\chi^2$ tests, and differences among levels of categorical predictors will be further examined through individual Z-tests. Only variables showing significant differences in heart attack odds across levels will be retained.
