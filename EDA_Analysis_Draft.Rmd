...
- Slope interpretation: $\hat{\beta}_{BMI} = 2.196e-04$. An one unit ($kg/m^2$) increase in BMI increases the odds of having heart attack by a multiplicative factor of $e^{2.196e-04} = 1.00022$.  increases the odds by 0.022%.
<br><br>
...
**Conclusion**: From the anova table, all the independent variables show significant relationship with odds of having diabetes. Sex, age, race, and income level have p-values < 2.2e-16. Healthcare coverage has a p-value is 0.0004477. 
<br><br>
...
2) **Age**: People with obesity have higher odds of having heart attack

- Individual Z test: p-value is < 2e-16

- Slope interpretation: $\hat{\beta}_{Age} = 6.015e-02$. In this model, when keeping every other conditions the same, an one year increase in age increases the odds of having diabetes by a multiplicative factor of $e^{6.015e-02} = 1.06197$, it increases the odds by 6.197%.
<br><br>

3) **Income Level**: Odds of having diabetes increases when income level increases. 
...
4) **Race**: white people has lowest odds of having diabetes. Within defined race groups, black people has highest odds of having diabetes
...
5) **HealthCare Coverafe**: Having healthcare covrage have higher odds of having heart attack
...
```{r}
glm_final <- glm(HeartAttack ~ BMI_Num + SEX + AGE + RACE + INCOME + HealthCare + Exercise + BMI_Num:RACE, data = df_clean, family = binomial(logit))
```
