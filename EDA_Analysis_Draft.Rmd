...
- **RACE**: Race group (recoded into 3 categories).  
...
- **BloodPressure**: Ever told by a health professional that they have high blood pressure.  
  - 1 = Yes - 2 = Yes, only during pregnancy - 3 = No - 4 = Borderline/pre-hypertensive

- **HealthCare**: Whether respondent currently has any health care coverage.  
  - 1 = Yes - 2 = No 

- **Exercise**: Whether the respondent did any physical activity (excluding work) in the past 30 days.  
  - 1 = Yes - 2 = No  
<br><br>
...
Among 10 variables, 9 of them are categorical variables stored in numbers. Thus, we first redefine the predictors to their corresponding categorical levels for a clearer representation Then we rename our predictors to make their names more comprehensible. By using the factor() function and specifying our own labels, we rename and redefine our selected variables.
...
**Conclusion**: There is an overwhelming more proportion of subjects that picked the category "Yes" (40.5%), but the mode still is "Borderline" (57.7%), which means that while we do not have many subjects that are in the "No" category, there are a lot in-between that might tip the distribution to be more equal than we initially thought.
<br><br>
...
**Conclusion**: Based on the mode calculation, Yes is dominantly observed over male (74.4% vs. 25.6%).This is another thing to take note of when we do further data analysis later on, in which the "No" cateogry is not fairly represented in this database, which can very well skew results.
<br><br>
...
**Conclusion**: When looking at the bar plot we made, we do not see a noticeable change in proportions of individuals having heart attack between the different categories of the Race predictor. It suggests that race may not be a significant risk factor of heart attack.
<br><br>
...
**Conclusion**: We do not see much of a difference in proportion of heart attacks across the different categories of BMI. It should be noted that the BMI predictor did not have many samples in the categories of "Obese" and "Overweight", which could lead to a biased statistic that needs more samples in order to truly tell us if there is no correlation.
<br><br>
...
**Conclusion**: There is a correlation between the different categories of Blood Pressure and the proportion of heart attacks. The "Yes" category has a much higher proportion of heart attacks than the others, with a value of 0.107 compared to 0.019, 0.025, and 0.037, which is a major different in proportions.
<br><br>
...
**Conclusion**: There doesn't appear to be a correlation between Healthcare Coverage and the proportion of heart attacks. Both categories have very similar proportions (5.9% vs. 3.8%), with only a slight increase for the "Yes" category, but not a statistically significant one. Therefore, we deduce there is not a correlation with this predictor
<br><br>
...
**Conclusion**: We do see a correlation between Exercise activity and the proportion of heart attacks. The "No" category has double the proportion of heart attacks as the "Yes" category, which leads us to conclude that there is a strong correlation between the activity and excerise rate and heart attacks.
<br><br>
...
### IV. Future Directions
  ...
  In the bar graphs, for each predictor, we visualize proportion of individuals with heart attack across different categories. Based on this preliminary analysis, we have observed correlations between 5 selected predictors with response variable **HeartAttack**. Significant predictors include **SEX**, **Income**, **AGE**, **BloodPressure**, and **Exercise**. Proportions of individuals with heart attack vary across different categorical levels, and it is observed in these predictors.  

  Specifically, we set **AGE** as the predictor in the main model. The significance of the predictor’s relationship with heart attack will be assessed using individual Z-tests. Other categorical factors will add as additive predictors in the extended model, which include **SEX**, **Income**, **BloodPressure**, and **Exercise**. Because they are categorical variables, we will analyze how the odds of having heart attack change when the category changes within each variable. Their overall contribution will first be evaluated using partial $\chi^2$ tests, and differences among levels of categorical predictors will be further examined through individual Z-tests. Only variables showing significant differences in heart attack odds across levels will be retained.
  
  Based on the significance results from both individual Z-tests and partial $\chi^2$ tests, the final extended model will be constructed. Model selection will be guided by the **Bayesian Information Criterion (BIC)**, which is appropriate given the large sample size and its preference for more parsimonious models. A **backward stepwise algorithm** will be used to identify the model with the lowest BIC.
  
  To ensure robustness, the final model will be assessed for influential data points using Cook’s distance and outlier tests for regression outliers. Multicollinearity between multiple predictors in the extensive models will be evaluated based on **Variance Inflation factor (VIF)**. The final model’s performance will be validated through two goodness-of-fit measures: the Hosmer-Lemeshow test and an error rate test, both of which assess how well the model predicts the actual odds of diabetes.
<br><br>
...
- Slope interpretation: $\hat{\beta}_{BMI} = 2.196e-04$. An one unit ($kg/m^2$) increase in BMI increases the odds of having heart attack by a multiplicative factor of $e^{2.196e-04} = 1.00022$. Having obesity increases the odds by 0.022%.
<br><br>

#### 2) Extended Model: Heart Attack vs. BMI + Demographic predictors

While BMI shows a significant relationship with odds of having heart attack. Other factors like demographic predictors are also connected to diabetes. Specifically, sex, age, income level, insurance coverage, race, and physical activity are included in the extended model based on the main model with BMI. Their relationship with odds of having heart attack is tested. 
<br><br>
...
```{r}
glm_extend <- glm(HeartAttack ~ BMI_Num + SEX + AGE + RACE + INCOME + HealthCare + Exercise, data = df_clean, family = binomial(logit))
...
**Conclusion**: From the anova table, all the independent variables show significant relationship with odds of having diabetes. Sex, age, race, income level, and physical activity have p-values < 2.2e-16. Healthcare coverage has a p-value is 0.0004477. 
<br><br>

**(c)** Individual Z Test (Need Revision)
<br><br>
**Purpose**: From the partial $\chi^2$ Test, all the independent variables are significantly related with odds of having diabetes. An individual Z Test is further preformed to check differences in odds of having diabetes within levels of each independent variable.

```{r}
#Baseline Code for Dummy variables
contrasts(df_clean$SEX)
contrasts(df_clean$AGE)
contrasts(df_clean$RACE)
contrasts(df_clean$INCOME)
contrasts(df_clean$HealthCare)
contrasts(df_clean$Exercise)
```
...
1) **Obesity**: People with obesity have higher odds of having diabetes

- Individual Z test: p-value is < 2e-16

- Slope interpretation: after adding demographic variables, the slope of obesity is still significantly related to odds of having diabetes. Thus, the extended model still retains the effectiveness in predicting odds of having diabetes. Specifically, $\hat{\beta}_{obesity} = 1.1287482$. In this model, when keeping every other conditions the same, individuals with obesity have $e^{1.1287482} = 3.09$ times higher odds of having diabetes compared to individuals without obesity.
<br><br>
2) **Sex**: Women have higher odds of having diabetes

- Slope interpretation: $\hat{\beta}_{Sex} = -0.0418418$. When keeping every other conditions the same, female have $e^{-0.0418418} = 0.959$ times lower odds of having diabetes compared to males. Being a female decreases the odds by 4.1%.
<br><br>
3) **Education level**: No significant differences in odds of having diabetes between education levels

- Slope interpretation: when comparing to the baseline group (*No schooling or kindergarten*), no education levels show significant differences in odds of having diabetes.
<br><br>
4) **Income Level**: Odds of having diabetes increases when income level increases. 

- **$15,000–$24,999 level**: when comparing to the baseline group (*Less than $15,000*), *$15,000–$24,999* income level has $e^{-0.2525002} = 0.776855$ times lower odds of having diabetes, it has a lower odds by 22.3%.

- **$25,000–$34,999 level**: when comparing to the baseline group (*Less than $15,000*), *$25,000–$34,999* income level has $e^{-0.4287925} = 0.651295$ times lower odds of having diabetes, it has a lower odds by 34.9%.

- **$35,000–$49,999 level**: when comparing to the baseline group (*Less than $15,000*), *$35,000–$49,999* income level has $e^{-0.5323012} = 0.587252$ times lower odds of having diabetes, it has a lower odds by 41.27%.

- **$50,000 or more level**: when comparing to the baseline group (*Less than $15,000*), *$50,000 or more* income level has $e^{-0.7374345} = 0.47834$ times lower odds of having diabetes, it has a lower odds by 52.17%.
<br><br>
5) **Race**: white people has lowest odds of having diabetes. Within defined race groups, black people has highest odds of having diabetes

- **Black**: when comparing to the baseline group (*White*), black people has $e^{0.5578105} = 1.74684$ times higher odds of having diabetes, it has a higher odds by 74.68%.

- **Other**: when comparing to the baseline group (*White*), people from other race groups has $e^{0.5610085} = 1.75244$ times higher odds of having diabetes, it has a higher odds by 75.24%.

- **Multiracial**: when comparing to the baseline group (*White*), multiracial people has $e^{0.4114635} = 1.50903$ times higher odds of having diabetes, it has a higher odds by 50.90%.

- **Hispanic**: when comparing to the baseline group (*White*), Hispanic people has $e^{0.4121775} = 1.5101$ times higher odds of having diabetes, it has a higher odds by 51.01%.
<br><br>
6) **Age**: People with obesity have higher odds of having diabetes

- Individual Z test: p-value is < 2e-16

- Slope interpretation: $\hat{\beta}_{Age} = 0.0404160$. In this model, when keeping every other conditions the same, an one year increase in age increases the odds of having diabetes by a multiplicative factor of $e^{0.0404160} = 1.04124$, it increases the odds by 4.04%.
<br><br>
...
#### 3) Extended Model: Diabetes vs. Obesity + Demographic predictors + Interaction term
Partial $\chi^2$ test is conducted to test if each interactive term is specifically related with odds of having diabetes. Individual levels within each qualitative variable is combined and tested together.  
```{r}
glm_full <- glm(HeartAttack ~ BMI_Num + SEX + AGE + RACE + INCOME + HealthCare + Exercise + BMI_Num:SEX + BMI_Num:AGE + BMI_Num:RACE + BMI_Num:INCOME + BMI_Num:HealthCare + BMI_Num:Exercise, data = df_clean, family = binomial(logit))

Anova(glm_full, type = "III")

glm_full_revised <- glm(HeartAttack ~ BMI_Num + SEX + AGE + RACE + INCOME + HealthCare + Exercise + BMI_Num:AGE + BMI_Num:RACE + BMI_Num:Exercise, data = df_clean, family = binomial(logit))
```

**Conclusion**: From the anova table, BMI shows significant interaction effects with age, race, and physical activity, suggesting that the impact of BMI on heart attack risk is not uniform across subgroups. Specifically, the relationship between BMI and the odds of having heart attack is moderated by age, race, and physical activity. Therefore, it cannot be interpreted independently of these factors.
<br><br>
...
\hat{\beta}_7 \cdot (\texttt{BMI} \times \texttt{Age}) +
\hat{\beta}_8 \cdot (\texttt{BMI} \times \texttt{Race})+
\hat{\beta}_8 \cdot (\texttt{BMI} \times \texttt{Physical Activity})+
$$

#### 4) Goodness of Fit Test

Purpose: after deriving the final model that is the best at predicting odds of having heart attack, two goodness of fit tests are conducted. They aim to assess how well the model predicts the actual odds of heart attack 

**(a)** The Hosmer-Lemeshow Test
```{r}
glm_final <- glm(HeartAttack ~ BMI_Num + SEX + AGE + RACE + INCOME + HealthCare + Exercise + BMI_Num:AGE + BMI_Num:RACE + BMI_Num:Exercise, data = df_clean, family = binomial(logit))

hoslem.test(df_clean$HeartAttack, fitted(glm_final), g = 20)
```
$H_0$: Fit is sufficient

$H_a$: Fit is not sufficient

P-value: < 2.2e-16

**Conclusion**: The p-value is < 2.2e-16, which is less than the significance level of 0.05. Thus, we reject the null hypothesis, and the fit of the logistic model is not sufficient for predicting odds of having diabetes.
<br><br>

**(b)** Error Rate
```{r}
glm.probs <- predict(glm_final, type = "response")
dim(df_clean)
...
#error rate
error <- (18412+9)/315938
error
```

**Conclusion**: The error rate is `r error`, which is around 0.05. Thus, the logistic model is good for predicting odds of having diabetes. 
<br><br>
