...
2. **Loading Dataset**: We first load our original dataset **2015.csv** into R Studio as data frame **df**.
...
### IV. Next Step
  ...
  Our preliminary analysis revealed clear differences in heart attack prevalence across `BMI` levels and demographic groups. People with higher `BMI` were more likely to report having had a heart attack, and similar patterns appeared for older `AGE`, lower `INCOME`, being male (`SEX`), and certain `RACE` groups. These patterns suggest that `BMI` could be an important predictor of heart attack risk. However, factors like age or income might also be linked to both `BMI` and heart attack risk at the same time -- meaning they could influence the relationship we see between `BMI` and heart attacks. To give a hypothetical example, being male could be associated with a higher chance of arterial stiffening and a tendency to have lower `BMI` on average. If we didn’t account for `SEX`, we might mistakenly think `BMI` has a weaker or stronger effect than it really does. Controlling for such confounding variables helps us isolate the effect of `BMI` from the effects of these other factors.

  Building on these findings, the main objective of this project is to model the odds of `HeartAttack` with `BMI` as the primary predictor, while controlling for potential confounders -- `SEX`, `INCOME`, `AGE`, and `RACE` -- identified from the EDA. This approach allows us to quantify the association between `BMI` and `HeartAttack` risk while accounting for demographic differences observed in the descriptive analysis.

  In the main logistic regression model, `BMI` will be the sole predictor, with its statistical significance assessed using individual Z-tests (a method that checks if the effect we see is large enough to be unlikely due to random chance). The extended model will add `SEX`, `INCOME`, `RACE`, and `AGE` as covariates. Because these covariates are categorical variables instead of continuous values, we will evaluate how the odds of `HeartAttack` change between categories. Their overall contribution will be assessed with partial $\chi^2$ tests, which compare models with and without a variable to see if adding it improves prediction, and differences between levels will be further examined through individual Z-tests. Variables that do not show significant differences will be excluded from the final model.

  The final extended model will be validated for robustness: multicollinearity will be checked using Variance Inflation Factor (VIF) to see if predictors are too strongly related to each other, which can distort results. Model performance will be evaluated using the error rate test.
