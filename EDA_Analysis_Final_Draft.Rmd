...
Our response variable Heart Attack is named as `CVDINFR4` in the original dataset and include 4 categories: 1 = No, 2 = Yes, 7 = Don't Know, 9 = Refused. We first factor the variable using function factor(). Specifically, we relabel 1 and 2 into No and Yes, and we relabel 7 and 9 into NA. Then we store the relabeled variable into a new variable `HeartAttack`.
```{r}
# Recode CVDINFR4
...
Our main predictor Body Mass Index (BMI) is named as `X_BMI5` in the original dataset is numeric discrete variable. We first factor the variable using function factor(). Specifically, we relabel 1 and 2 into No and Yes, and we relabel 7 and 9 into NA. Then we store the relabeled variable into a new variable `HeartAttack`
```{r}
# Recode _BMI5 
